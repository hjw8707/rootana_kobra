{
  ...
  ko.DrawMomDist(isotope, true);

  KOBRA ko2(576,579);
  ko2.SetBrho(1.5065);
  ko2.SetUsePla();
  ko2.ApplyOffsetToCut(-0.025,-0.2,-1,1.2);
  ko2.DrawMomDist(isotope, true);

  auto h1 = static_cast<TH1*>(gDirectory->Get(Form("hmd_r572_%s",isotope)));
  auto h2 = static_cast<TH1*>(gDirectory->Get(Form("hmd_r576_%s",isotope)));
    
  TGraphErrors *gr1 = new TGraphErrors;
  gr1->SetTitle(Form("Momentum Distribution of %s;Momentum Dispersion (%);Rate (pps)",isotope));
  ...
  //  ko.DrawMomDist(isotope);
  //  ko2.DrawMomDist(isotope);
}
