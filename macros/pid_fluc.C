...
void pid_fluc() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        ko->SetBrho(brho[i]);
        auto nEvent = ko->tree->GetEntries();
        ko->tree->Draw(Form("aoq:Entry$>>h_%s%d(200,0,%lld,200,%f,%f)", z[i], a[i], nEvent,
                            flagCut ? static_cast<double>(a[i]) / zz[i] - 0.1 : 2,
                            flagCut ? static_cast<double>(a[i]) / zz[i] + 0.1 : 3),
                       flagCut ? cut[i] : 0, "col");

        gPad->GetCanvas()->Print(Form("figs/pid_fluc_%s%d%s.png", z[i], a[i], flagCut ? "_cut" : ""));
        delete ko;
    }
}