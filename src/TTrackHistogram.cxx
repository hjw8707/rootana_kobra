...
void TTrackHistograms::UpdateHistograms(TDataContainer &) {
    ...
    if (ppacdn->GetEntriesFast() > 0) GetHistogram(3)->Fill((ppacup->GetEntriesFast() > 0));
    if (ppacup->GetEntriesFast() > 0) GetHistogram(4)->Fill((ppacdn->GetEntriesFast() > 0));
}

/// Take actions at begin run
void TTrackHistograms::BeginRun(int transition, int run, int time) { CreateHistograms(); }
