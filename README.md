...
두 데이터의 시간 기준이 어긋나 있으면 **타임스탬프 오프셋**을 줄 수 있습니다.

```bash
# BeamDump를 +2초 이동(=더 늦게 표시)해서 LEBT와 정렬
python scripts/plot_lebt_beamdump.py \
  --date-start 2024-08-13 --time-start 13:05:00 --time-end 13:50:00 \
  --offset-beamdump +2s
```

오프셋을 “자동으로” 찾고 싶으면 `--auto-offset`을 사용하세요(범위/스텝을 스캔해서 Pearson 상관이 최대가 되는 \(\Delta t\)를 찾습니다).
...
LEBT-BeamDump **2D correlation(상관) 플롯**을 그리고 싶으면 `--correlation`을 사용하세요. 내부적으로 timestamp 기준으로 두 데이터를 병합(`merge_asof`)하며, `--tolerance`로 허용 시간차를 조절합니다.
...
`pars/` 디렉토리에는 각 검출기(Detector)의 보정 파라미터가 텍스트 파일 형식으로 저장되어 있습니다. 이 파일들은 `analist.txt`를 통해 각 검출기 분석 클래스에 로드되며, 원시 데이터를 물리량으로 변환하는 데 사용됩니다.
...
**형식 (5줄):**
```
줄 0: chs          채널 번호들 (공백 구분)
줄 1: offset       각 채널별 오프셋 값 (공백 구분)
줄 2: factor       각 채널별 보정 계수 (공백 구분)
줄 3: tdc_cut      TDC 하한값 상한값 (2개 값)
줄 4: tsum_cut     X 하한값 X 상한값 Y 하한값 Y 상한값 (4개 값)
```

**설명:**
- `chs`: 검출기에서 사용하는 TDC 채널 번호 목록
- `offset`: 각 채널의 시간 오프셋 보정값 (ns 단위)
- `factor`: 각 채널의 시간 보정 계수 (ns/ch 단위)
- `tdc_cut`: TDC 값의 유효 범위 [하한값, 상한값]
- `tsum_cut`: X, Y 위치 합(tsum)의 유효 범위 [X 하한, X 상한, Y 하한, Y 상한]

**사용 클래스:** `TPPACAna`

### 7.2. LPPAC (Large PPAC) 파라미터 파일
...
줄 3: tdc_cut      TDC 하한값 상한값 (2개 값)
줄 4: tsum_cut     X 하한값 X 상한값 Y 하한값 Y 상한값 (4개 값)
줄 5: txsum_offset X 위치 합 오프셋 [left, center, right] (3개 값)
```

**설명:**
- PPAC와 동일하지만 추가로 `txsum_offset` 파라미터가 있음
- `txsum_offset`: X 위치 합의 왼쪽, 중앙, 오른쪽 오프셋 보정값

**사용 클래스:** `TLPPACAna`

### 7.3. LPPACX (Large PPAC X-only) 파라미터 파일
...
줄 2: factor       각 채널별 보정 계수 (공백 구분)
줄 3: tdc_cut      TDC 하한값 상한값 (2개 값)
줄 4: tsum_cut     X 하한값 X 상한값 (2개 값)
줄 5: txsum_offset X 위치 합 오프셋 [left, center, right] (3개 값)
```

**설명:**
- LPPAC와 유사하지만 X 방향만 측정하므로 `tsum_cut`가 2개 값만 있음
...
줄 2: factor       각 채널별 보정 계수 (공백 구분)
줄 3: tdc_cut      TDC 하한값 상한값 (2개 값)
줄 4: tdiff_cut    시간 차이(tdiff) 하한값 상한값 (2개 값)
```

**설명:**
- `chs`: Plastic 검출기에서 사용하는 TDC 채널 번호 목록
- `offset`: 각 채널의 시간 오프셋 보정값
- `factor`: 각 채널의 시간 보정 계수
- `tdc_cut`: TDC 값의 유효 범위
- `tdiff_cut`: 좌우 채널 간 시간 차이(tdiff)의 유효 범위

**사용 클래스:** `TPlasticAna`

### 7.5. DPlastic (Digitizer Plastic) 파라미터 파일
...
```
줄 0: chs          채널 번호들 (공백 구분)
줄 1: t_offset     시간 오프셋 값들 (공백 구분)
줄 2: t_factor     시간 보정 계수들 (공백 구분)
줄 3: a_offset     에너지(ADC) 오프셋 값들 (공백 구분)
줄 4: a_factor     에너지(ADC) 보정 계수들 (공백 구분)
줄 5: tdc_cut      TDC 하한값 상한값 (2개 값)
줄 6: adc_cut      ADC 하한값 상한값 (2개 값)
```

**설명:**
- `chs`: DPlastic 검출기에서 사용하는 DIG 채널 번호 목록
- `t_offset`: 각 채널의 시간 오프셋 보정값
- `t_factor`: 각 채널의 시간 보정 계수
- `a_offset`: 각 채널의 에너지(ADC) 오프셋 보정값
- `a_factor`: 각 채널의 에너지(ADC) 보정 계수
- `tdc_cut`: TDC 값의 유효 범위
- `adc_cut`: ADC 값의 유효 범위

**사용 클래스:** `TDPlasticAna`

### 7.6. SSD (Silicon Strip Detector) 파라미터 파일
...
```
줄 0: chs          채널 번호들 (공백 구분)
줄 1: ped          각 채널별 pedestal 값 (공백 구분)
줄 2: offset       각 채널별 오프셋 값 (공백 구분)
줄 3: factor       각 채널별 보정 계수 (공백 구분)
줄 4: adc_cut      ADC 하한값 상한값 (2개 값)
```

**설명:**
- `chs`: SSD 검출기에서 사용하는 ADC 채널 번호 목록
- `ped`: 각 채널의 pedestal 값 (기저선 보정)
- `offset`: 각 채널의 에너지 오프셋 보정값
- `factor`: 각 채널의 에너지 보정 계수
- `adc_cut`: ADC 값의 유효 범위
...
파라미터 파일은 `analist.txt`에서 각 검출기에 매핑됩니다:

```
검출기이름  데이터타입  뱅크이름  분석타입  파라미터파일경로  [채널수]
```
...
파라미터 파일은 `#`으로 시작하는 주석을 지원하며, 빈 줄은 무시됩니다.
